...
- (void)start
{
  ...
  [self loadSource:^(NSError *error, RCTSource *source) {
    printf("guangy source code loaded");
    if (error) {
      [weakSelf handleError:error];
    }
...
-(void)loadModule:(NSString*)moduleName url:(NSURL*)url onComplete:(void(^)(NSError *error))onComplete{
  [RCTJavaScriptLoader loadBundleAtURL:url onProgress:^(__unused RCTLoadingProgress *progressData) {
    
  } onComplete:^(NSError *error, RCTSource *source) {
    if (error) {
      onComplete(error);
    } else {
      [self enqueueApplicationScript:source.data url:url onComplete:^{
        dispatch_async(dispatch_get_main_queue(), ^{
          [[NSNotificationCenter defaultCenter]
           postNotificationName:RCTModelDidLoadNotification
           object:self->_parentBridge userInfo:@{@"bridge": self, @"moduleName":moduleName}];
          onComplete(NULL);
        });
      }];
    }
